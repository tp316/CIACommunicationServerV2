...
protected:
	...
	void init_cti();
	void init_hungup_strategy();

private:
	std::size_t                            m_numChannelCount;                       //中继线总数
	vector<hungup_strategy>                m_vec_hungup_strategy;	                //保存全部呼叫挂断延迟策略的数组
	...
	thread_group                           m_hungup_strategy_thread_group;	        //执行延迟策略的线程池
};

vector<boost::shared_ptr<trunk>>  voice_card_control::m_trunk_vector;            // 保存中继状态数组, 由于在trunk中使用了NO_COPYABLE的类型 mutex, 所以此处存储shared_ptr
...
{
	m_numChannelCount = numChannelCount;
	m_timeout_elapse = timeout_elapse;
	for (size_t i = 0; i < numChannelCount; i++)
	{
		m_trunk_vector.push_back(boost::make_shared<trunk>());
	}
	init_cti();
	if (load_strategy)
		init_hungup_strategy();
	// 创建延迟策略线程
	for (size_t i = 0; i <= numChannelCount / 3 / 2; i++) // 除3:三大运营商 除2:二分之一使用4G, 根据目前为止的测试, 仅有联通4G用户需要使用策略
	{
...
void voice_card_control::init_cti()
{
	...
	if (SsmStartCti("ShConfig.ini", "ShIndex.ini") != 0)
	{
		...
		throw runtime_error("读取语音卡配置文件失败");
	}
	else{
	...
	EventMode.lpHandlerParam = voice_card_control::cti_callback;          //注册回调函数
...
void voice_card_control::init_hungup_strategy()
{
	ifstream finput("hungup_strategy.config");
	string file_line;
	while (getline(finput, file_line)) {
		hungup_strategy hs;
		istringstream s_line(file_line);
		s_line >> hs.m_phone_number_head;
		s_line >> hs.m_warning_elapse;
		s_line >> hs.m_sleeping_elapse;
		m_vec_hungup_strategy.push_back(hs);
	}
	finput.close();
	BOOST_LOG_SEV(cia_g_logger, RuntimeInfo) << "呼叫挂断延迟策略加载完毕, 共加载" << m_vec_hungup_strategy.size() << "条策略";
}
...
int voice_card_control::timeout_check()
{
	...
	while (true)
	{
		...
		{
			trunk& trunk = *(m_trunk_vector.at(i));
...
int voice_card_control::cti_callout(boost::shared_ptr<base_client> client_socket, string transId, string authCode, string pn, bool hungup_by_echo_tone)
{
	std::size_t chID;
	try
	{
	...
	for (hungup_strategy& strategy : m_vec_hungup_strategy)
	{
		if (strategy.equals(pn)){
			m_trunk_vector.at(chID)->m_hungup_strategy = &strategy;
			break;
		}
	}
	...
	if (SsmAutoDial(chID, pn.c_str()) == 0){
		...
		trunk& t = *(m_trunk_vector.at(chID));
...
int voice_card_control::cti_hangUp(std::size_t channelID, string status)
{
	int retVal = 0;
	trunk& t = *(m_trunk_vector.at(channelID));
	if (!t.m_hungup_by_echo_tone)
	{
		BOOST_LOG_SEV(cia_g_logger, CalloutMsg) << "业务流水:" << t.m_transId << " 由于设置非响一声挂断, 未挂断本次呼叫请求, 通道号码: " << channelID;
		return -1;
	}
	if (t.m_step == TRK_IDLE){
		BOOST_LOG_SEV(cia_g_logger, Warning) << "业务流水:" << t.m_transId << " 可能出现重复挂机, 请在后续版本调查原因, 通道号码: " << channelID;
		return -1;
	}
...
int CALLBACK voice_card_control::cti_callback(WORD wEvent, int nReference, DWORD dwParam, DWORD dwUser)
{
	try{
		...
		case E_SYS_NoSound:
			...
			return 1;
			break;
		case E_RCV_Ss7Msu:	            // 在E_RCV_Ss7Msu事件中, 参数 nReference dwUser 均不使用, 函数SsmGetSs7Msu用于取出消息
			// nReference 不使用
...
int voice_card_control::deal_e_proc_auto_dial(int nReference, DWORD dwParam, DWORD dwUser)
{
	...
	case DIAL_ECHOTONE:	            // TUP/ISUP通道：表明驱动程序收到对端交换机的地址齐消息(ACM)
		...
		{
			trunk& t = *(m_trunk_vector.at(nReference));
			if (t.m_hungup_strategy != nullptr)//通道此次呼叫有延迟策略
			{
				if (t.m_callTime.elapsed() <= t.m_hungup_strategy->m_warning_elapse)   // 触发挂机的时间比延迟策略规定的时间短
				{
...
void voice_card_control::deal_hungup_strategy()
{
	...
	while (true)
	{
		...
		int sleeping_elapse = m_trunk_vector.at(chID)->m_hungup_strategy->m_sleeping_elapse - (int)(m_trunk_vector.at(chID)->m_callTime.elapsed() * 1000);
